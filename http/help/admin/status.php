...
if(isset($_POST['edit_bid_status'])){
  ...
  if (isset($_POST["bid_contractor"])) {$bid_contractor = clean($_POST['bid_contractor']);} else {echo '<div style="text-align:center;">O_o</div>'; exit();}
  if (isset($_POST["bid_priority"])) {$bid_priority = clean($_POST['bid_priority']);} else {echo '<div style="text-align:center;">O_o</div>'; exit();}
  

  $result=mysqli_query($link,"UPDATE bid SET status='.$bid_status_edit.',contractor='.$bid_contractor.',priority='.$bid_priority.',alarm_update=1 WHERE id='".$bid."' LIMIT 1");
  if ($result=='true') {
    if (isset($_POST['search'])) {
      echo '<div class="alert alert-success text-center" role="alert">Статус заявки обновлён.</div></br>
      <div style="text-align:center;">
        <a href="/help/admin/status.php?page='.$page.'&bid='.$bid.'&details&search">
      ...
      <meta http-equiv="refresh" content="1;url=/help/admin/status.php?page='.$page.'&bid='.$bid.'&details&search">
      ';
    } else {
      echo '<div class="alert alert-success text-center" role="alert">Статус заявки обновлён.</div></br>
      <div style="text-align:center;">
        <a href="/help/admin/status.php?page='.$page.'&bid='.$bid.'&details">
          <button type="submit" class="btn btn-success">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-90deg-up" viewBox="0 0 16 16">
              <path fill-rule="evenodd" d="M4.854 1.146a.5.5 0 0 0-.708 0l-4 4a.5.5 0 1 0 .708.708L4 2.707V12.5A2.5 2.5 0 0 0 6.5 15h8a.5.5 0 0 0 0-1h-8A1.5 1.5 0 0 1 5 12.5V2.707l3.146 3.147a.5.5 0 1 0 .708-.708l-4-4z"></path>
            </svg>
      ...
      <meta http-equiv="refresh" content="1;url=/help/admin/status.php?page='.$page.'&bid='.$bid.'&details">
      ';
    }
...
if(isset($_POST['edit_bid_category'])) {
    //Проверим данные из формы
    if (isset($_POST["page"])) {$page = clean($_POST['page']);} else {echo '<div style="text-align:center;">O_o</div>'; exit();}
    if (isset($_POST["bid"])) {$bid = clean($_POST['bid']);} else {echo '<div style="text-align:center;">O_o</div>'; exit();}
    if (isset($_POST["bid_filial"])) {$bid_filial = clean($_POST['bid_filial']);} else {echo '<div style="text-align:center;">O_o</div>'; exit();}
    if (isset($_POST["bid_tempate"])) {$bid_tempate = clean($_POST['bid_tempate']);} else {echo '<div style="text-align:center;">O_o</div>'; exit();}
    
    $result=mysqli_query($link,"UPDATE bid SET filial='$bid_filial',template='$bid_tempate' WHERE id='".$bid."' LIMIT 1");
    if ($result=='true') {
      echo '<div class="alert alert-success text-center" role="alert">Категория заявки обновлена.</div></br>
      <div style="text-align:center;">
        <a href="/help/admin/status.php?page='.$page.'&bid='.$bid.'&details">
          <button type="submit" class="btn btn-success">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-90deg-up" viewBox="0 0 16 16">
      ...
      echo '<div class="alert alert-danger text-center" role="alert">Ошибка обновления категории заявки. Что то пошло не так.</div></br>
      <div style="text-align:center;">
        <a href="/help/admin/status.php?page='.$page.'&bid='.$bid.'&details">
          <button type="submit" class="btn btn-warning">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-90deg-left" viewBox="0 0 16 16">
...
if(isset($_POST['send_message'])){
  ...
  <div style="text-align:center;">
    <a href="/help/admin/status.php?page=1"><button type="submit" class="btn btn-warning">
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-90deg-left" viewBox="0 0 16 16">
      <path fill-rule="evenodd" d="M1.146 4.854a.5.5 0 0 1 0-.708l4-4a.5.5 0 1 1 .708.708L2.707 4H12.5A2.5 2.5 0 0 1 15 6.5v8a.5.5 0 0 1-1 0v-8A1.5 1.5 0 0 0 12.5 5H2.707l3.147 3.146a.5.5 0 1 1-.708.708l-4-4z"></path>
      </svg>
      Вернуться назад
  ...
  if ($_FILES["userfile"]['size']>0){
    ...
    if ($_FILES['userfile']['size']>$file_size_max) {
      ...
      <div style="text-align:center;">
        <a href="/help/admin/status.php?page='.$page.'&bid='.$bid.'&details"><button type="submit" class="btn btn-warning">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-90deg-left" viewBox="0 0 16 16">
          <path fill-rule="evenodd" d="M1.146 4.854a.5.5 0 0 1 0-.708l4-4a.5.5 0 1 1 .708.708L2.707 4H12.5A2.5 2.5 0 0 1 15 6.5v8a.5.5 0 0 1-1 0v-8A1.5 1.5 0 0 0 12.5 5H2.707l3.147 3.146a.5.5 0 1 1-.708.708l-4-4z"></path>
          </svg>
          Назад
...
if(isset($_GET['details'])){
  ...
  $query = mysqli_query($link,"SELECT id,priority,template,filial,theme,status,owner,contractor,date,message FROM bid WHERE id='".$bid."' LIMIT 1");
  if (mysqli_num_rows($query) > 0) {
    $data = mysqli_fetch_assoc($query);

    $query_owner = mysqli_query($link, "SELECT id,f_name,s_name,unit,position,phone,email FROM users WHERE id='".$data['owner']."' LIMIT 1");
    if(mysqli_num_rows($query_owner) > 0){
      $data_owner = mysqli_fetch_assoc($query_owner);
      $user_id=$data_owner['id'];
      $f_name=$data_owner['f_name'];
      $s_name=$data_owner['s_name'];
      $unit=$data_owner['unit'];
      $position=$data_owner['position'];
      $phone=$data_owner['phone'];
    ...
    //Найдём исполнителья
    $contractor = '<option value="0">Не назначен</option>';
    $contractor_info = 'Не назначен';
    $query_contractor = mysqli_query($link,"SELECT id,f_name,s_name,position FROM users WHERE role in ('operator','admin','superadmin') and active=1 ORDER BY s_name ASC");
    while ($data_contractor = mysqli_fetch_assoc($query_contractor)) {
      if ($data['contractor'] == $data_contractor['id']) {
        $selected = 'selected';
        $contractor_info = $data_contractor['f_name'].' '.$data_contractor['s_name'];
      } else {
        $selected = '';
      }
      $contractor = $contractor.'<option value="'.$data_contractor['id'].'" '.$selected.'>'.$data_contractor['s_name'].' '.$data_contractor['f_name'].' '.$data_contractor['position'].'</option>';
    }
    ...
    if ($data['status']==1) {
      ...
      $status_edit_menu='
        <option value="2" selected>Назначить</option>
        <option value="3">Взять в работу</option>
        <option value="4">Отменить</option>
      ';
      ...
      $status_edit_menu='
        <option value="2" selected>Назначить</option>
        <option value="3" selected>Взять в работу</option>
        <option value="6">Запрос информации</option>
        <option value="4">Отменить</option>
      ';
      ...
      $status_edit_menu='
        <option value="2" selected>Назначить</option>
        <option value="6">Запрос информации</option>
        <option value="7">Приостановить</option>
        <option value="8">На согласование</option>
        <option value="9">В закупке</option>
        <option value="4" selected>Завершить</option>
      ';
      ...
      $status_edit_menu='
        <option value="3">Вернуть в работу</option>
    ...
    ';
      ...
      $status_edit_menu='
      <option value="2" selected>Вернуть в работу</option>
      <option value="4">Завершить</option>
      ';
      ...
      $status_edit_menu='
        <option value="2" selected>Согласовать</option>
        <option value="8">На согласование</option>
  ...
  $filial = '';
  $selected = '';
  $query_filial = mysqli_query($link, "SELECT id,name FROM bid_filials WHERE active=1");
  while ($data_filial = mysqli_fetch_assoc($query_filial)){
    if ($data_filial['id'] == $data['filial']){
      $selected = 'selected';
      $filial = $data_filial['name'];
    } else {$selected = '';}
    $filial_all = $filial_all.'<option value="'.$data_filial['id'].'" '.$selected.'>'.$data_filial['name'].'</option>';
  }
  ...
  $tempate= '';
  $selected = '';
  $query_tempates = mysqli_query($link, "SELECT id,name FROM bid_tempates");
  while ($data_tempates = mysqli_fetch_assoc($query_tempates)){
    if ($data_tempates['id'] == $data['template']){
      $selected = 'selected';
      $tempate = $data_tempates['name'];
    } else {$selected = '';}
    $tempates_all = $tempates_all.'<option value="'.$data_tempates['id'].'" '.$selected .'>'.$data_tempates['name'].'</option>';
  }
  ...
  $priority= '';
  $selected = '';
  $query_priority = mysqli_query($link, "SELECT id,name FROM bid_priority WHERE active=1 ORDER BY id DESC");
  while ($data_priority = mysqli_fetch_assoc($query_priority)) {
    if ($data_priority['id'] == $data['priority']){
      $selected = 'selected';
      $priority = $data_priority['name'];
    } else {$selected = '';}
    $priority_all = $priority_all.'<option value="'.$data_priority['id'].'" '.$selected .'>'.$data_priority['name'].'</option>';
  }
  ...
  <div class="container">
    <div style="text-align:center;">
      <a href="/help/admin/status.php?page=%s">
        %s
      </a>
    </div>
    </br>
      ...
      <div class="col col-md-4 py-3 border border-primary bg-light rounded-2">
        <div class="card">
          <div class="card-body text-center">
            <h5 class="card-title text-center">О заявке</h5>
        ...
        <div class="card text-center">
          <div class="card-body">
            <h5 class="card-title">Управление категорией</h5>
            <form action="" method="post">
              <div class="mb-3">
                <label for="Filial" class="form-label">Филиал</label>
                <select name="bid_filial" class="form-select" aria-label="...">
                  %s
                </select>
                <label for="Filial" class="form-label">Шаблон</label>
                <select name="bid_tempate" class="form-select" aria-label="...">
                  %s
                </select>
                </br>
                <input type="hidden" name="page" value="%s">
                <input type="hidden" name="bid" value="%s">
                <button name="edit_bid_category" class="btn btn-outline-primary">Изменить категорию</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div class="card text-center">
          <div class="card-body">
            ...
            <div class="mb-3">
              ...
              <label for="Status" class="form-label">Исполнитель</label>
              <select name="bid_contractor" class="form-select" aria-label="Default select example">
                %s
              </select>
              <label for="Status" class="form-label">Приоритет</label>
              <select name="bid_priority" class="form-select" aria-label="Default select example">
                %s
              </select>
              </br>
              </br>
              ...
              <input type="hidden" name="bid" value="%s">
              %s
              <button name="edit_bid_status" class="btn btn-outline-primary">Изменить статус</button>
            </div>
            </form>
          </div>
        </div>
  ...
  ',$page,$button_return,$bid,$bid_date,$data['theme'],$data['message'],$file,$message,$page,$bid,$history,$filial_all,$tempates_all,$page,$bid,$f_name.' '.$s_name,$unit,$position,$phone,$email,$status_color,$status_edit_menu,$contractor,$priority_all,$page,$bid,$search_status);
  goto end_load;
}
...
$result = mysqli_query($link,"SELECT COUNT(*) FROM bid");
...
?>
<div class="container">
  <h4 style="text-align:center;">Просмотр всех заявок</h3>
  ...
  <div class="row justify-content-start">
    <div class="col col-md-12">
      <table class="table text-center table-hover table-bordered table-light bg-light" border="1">
        ...
        <tbody>
          <?php
            $query = mysqli_query($link,"SELECT id,priority,filial,theme,date,status FROM bid ORDER BY id DESC LIMIT $start, $num");
            $i = $start + 1;
            while ($data = mysqli_fetch_assoc($query))
             {
              ...
              $data_filial = mysqli_fetch_assoc($query_filial);
               ...
               <tr>
                ...
                <td>%s</td>
                <td>%s</td>
                <td><a href="/help/admin/status.php?page=%s&bid=%s&details"><button type="button" name="details" class="btn btn-outline-primary">Подробнее</button></a></td>
               </tr>
               ',$data['id'],mb_strimwidth($data_filial['name'], 0, 15, "..."),mb_strimwidth($data['theme'], 0, 50, "..."),$bid_date,$priority,$status_color,$page,$data['id']);
               $i++;
             }
          ?>
        </tbody>
